    ...
    def execute(self):
        ...
        for n in range(len(self.actions)):
            ...
            while(process):         
                ...
                # if done != prev_done:
                #     new_done_time = time.time()
                #     print("Processed {} for {} in {}s.".format(done,action,new_done_time-done_time))    
                #     done_time = time.time()
        ...
        while(not self.kill_worker):
            if not self.stop_worker:
                if self.done < self.amount:
                    # if self.worker_number == 1 or self.worker_number == 8:
                        # print(self.worker_number ," prints: ",self.problems_list[self.done])
